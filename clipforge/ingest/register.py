...
def main(args) -> int:
    ...
    try:
        return _register(cfg, conn, args, master)
...
def _register(cfg, conn, args, master: Path) -> int:
    date = args.date or default_date(master)
    label = args.title or master.stem

    known = existing_ids(conn)
    if args.id:
        stream_id = args.id
        exists = stream_id in known
    else:
        # make_stream_id already handles slugging and same-day collisions.
    ...
    if exists and not args.force:
        print(
            f"stream {stream_id!r} is already registered. Re-registering can point it at a "
            f"different master, which invalidates every artifact built from the old one. "
            f"Pass --force to update it and re-run whatever depends on what changed."
        )
        return 1

    anchor_path = find_capture_file(master, args.anchor, "anchor.json")
    markers_path = find_capture_file(master, args.markers, "markers.jsonl")
    ...
    copied = _copy_capture_files(stream_paths, anchor_path, markers_path)
    _write_sources(stream_paths, master, anchor_path, markers_path)

    games = (
        json.dumps([g.strip() for g in args.games.split(",") if g.strip()])
    ...
    with db.transaction(conn):
        conn.execute(
            ...
            """,
            (stream_id, date, args.title, games, str(master), anchor_ms, time_base, args.notes),
        )

    engine = runner.Runner(cfg=cfg, conn=conn, stream_id=stream_id, log=lambda m: print(f"  {m}"))
    ...
    material = _material_change(previous, master, anchor_ms, time_base)
    if previous is None or material:
        engine.mark_external_done("register_stream", identity_hash(master, anchor_ms))
    else:
        print("  metadata only: leaving completed stages alone")

    _report(stream_id, master, date, anchor_ms, time_base, copied, previous, material)
    return 0
...
def _report(stream_id, master, date, anchor_ms, time_base, copied, previous, material) -> None:
    print(f"{'updated' if previous is not None else 'registered'}  {stream_id}")
    print(f"  master    {master}")
    print(f"  date      {date}")
    if anchor_ms is not None:
        started = datetime.fromtimestamp(anchor_ms / 1000).isoformat(timespec="seconds")
        print(f"  anchor    {anchor_ms}  ({started})")
    else:
        print("  anchor    none — marker times will be read as VOD seconds")
    print(f"  markers   {'markers.jsonl' if 'markers.jsonl' in copied else 'none found'}")

    if "markers.jsonl" in copied and time_base == "vod":
        print(
            "\n  WARNING  markers.jsonl is present but anchor.json is not. §4.3 markers carry\n"
            "           epoch milliseconds, and without the anchor there is nothing to convert\n"
            "           them against (§4.1). Register again with --anchor once you have it."
        )

    if previous is not None and material:
        print("\n  the master or anchor changed: dependent stages will re-run on the next `run`")

    print("\nnext: clipforge run " + stream_id)
