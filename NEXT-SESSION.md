...
**Phases 0, 1, 2 and 4 of §15 are complete. 1190 tests pass** (`.venv\Scripts\python.exe -m pytest -q`), plus 3 that need `--asr`.
...
**§13.2's nightly `VACUUM INTO` backup.** The spec calls the database the
irreplaceable tier — "signals can be re-extracted from footage; the operator's
judgment calls cannot" — and prescribes a nightly job. Nothing implements it
and there is no `data/backups/`. **This is the first thing to build the moment
there are real ratings to lose**, which is the moment you finish reviewing your
first stream.
...
| What | Why now | Cost |
|---|---|---|
| **§13.2's backup job** | The database is the irreplaceable tier and there is no backup. Small. | An hour |
| **Phase 3's nudge keys** (`[`/`]`/`{`/`}`) | GUESSES gap #1: §17 tunes `min_window_s` against "how often the operator nudges boundaries" and the keys do not exist, so the number cannot be collected. They are greyed in the UI already. | Half a day |
